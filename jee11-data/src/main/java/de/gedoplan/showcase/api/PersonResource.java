...
public class PersonResource {
  ...
  @Path("page")
  @Produces(MediaType.APPLICATION_JSON)
  public Map<String, List<Person>> getPage(@QueryParam("page") @DefaultValue("3") long page) {
    PageRequest pageRequest = PageRequest.ofPage(page).size(2);
    Order<Person> sortBy = Order.by(Sort.asc("id"));
    Map<String, List<Person>> result = new TreeMap<>();

    Page<Person> current = this.personRepository.findAll(pageRequest, sortBy);
    result.put("current", current.stream().toList());

    Page<Person> next = this.personRepository.findAll(current.nextPageRequest(), sortBy);
    result.put("next", next.stream().toList());
